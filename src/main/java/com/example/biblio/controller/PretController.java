...
public String rejeterPret(@PathVariable("id") int pretId, RedirectAttributes redirectAttributes) {
    ...
    public String reserverExemplaire(
        ...
        List<ReservationEnAttenteDTO> prets = statutPretService.getPretsEnCoursByAdherant(adherantId);
        int count = statutPretService.countPretsEnCoursByAdherant(adherantId);
        Adherant adherant = adherantService.getById(adherantId);
        int capacite = adherant.getTypeAdherant().getCapacite();
        int delai = adherant.getTypeAdherant().getDelai();
        TypePret typePret = typePretService.getById(2);
        Exemplaire exemplaire = exemplaireService.getById(exemplaireId.intValue());
    
        if (count >= capacite) {
            redirectAttributes.addFlashAttribute("reservationSuccess", false);
            redirectAttributes.addFlashAttribute("errorMessage", "Capacité d'emprunt dépassée (" + capacite + ").");
            return "redirect:/exemplaires/disponibles";
        }
    
        if (!pretService.isExemplaireDisponible(exemplaireId, reservationDate, delai)) {
            Optional<Pret> conflit = pretService.getPlageOccupee(exemplaireId, reservationDate, delai);
            if (conflit.isPresent()) {
                LocalDate debut = conflit.get().getDate_debut_prevision();
                LocalDate fin = debut.plusDays(delai);
                redirectAttributes.addFlashAttribute("reservationSuccess", false);
                redirectAttributes.addFlashAttribute("errorMessage", "Non disponible. Occupé du " + debut + " au " + fin + ".");
                return "redirect:/exemplaires/disponibles";
            }
        }
